...
export async function GET(request) {
  try {
    ...
    for (const a of assignments) {
      ...
      conversations.push({ id: candidateUserId, candidateName: `${a.candidateId.firstName} ${a.candidateId.lastName}`, lastMessage: lastMessage?.text || '', unreadCount });
    }
...
export async function POST(request) {
  try {
    ...
    const msg = { id: `msg_${Date.now()}`, conversation: 'coach', sender: 'coach', text: message, createdAt: new Date().toISOString() };
    workspace.messages = workspace.messages || [];
    workspace.messages.push(msg);

    workspace.notifications = workspace.notifications || [];
    workspace.notifications.push({ id: `notif_${Date.now()}`, title: 'New Message', message: message.slice(0, 200), type: 'message', href: '/candidate/messages', read: false });
