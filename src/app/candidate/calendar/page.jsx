  ...
  return (
    <div className="relative max-w-7xl mx-auto px-4 sm:px-6 pb-16 animate-in fade-in duration-500 font-['DM_Sans',sans-serif]">
      ...
      <style>{`
        ...
        .btn-ghost:hover { background: var(--primary-glow); border-color: var(--primary); color: var(--text-primary); }
        .btn-primary {
          display: flex; align-items: center; gap: 8px;
        ...
        .input-dark {
          ...
          padding: 12px 16px; color: var(--text-primary); font-size: 14px; outline: none;
          transition: all 0.2s; font-family: inherit;
        }
        .input-dark:focus { border-color: var(--primary); background: var(--primary-glow); box-shadow: 0 0 0 3px var(--primary-glow); }
      ...
      <div className="glass-card p-6 sm:p-8 mb-6 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          ...
          <h1 className="serif text-3xl md:text-4xl text-var(--text-primary) leading-tight mb-2">Program Calendar</h1>
          <p className="text-var(--text-muted) font-light text-sm sm:text-base">Manage your sessions with <span className="text-var(--primary) font-medium">{coachName}</span></p>
        </div>
        ...
        {viewMode === 'month' ? (
          <div className="space-y-4 sm:space-y-6">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-3 sm:gap-4 w-full sm:w-auto justify-between sm:justify-start">
                <h2 className="text-xl sm:text-2xl font-medium text-var(--text-primary) serif">
                  {new Date(currentDate.getFullYear(), currentDate.getMonth()).toLocaleString('default', { month: 'long', year: 'numeric' })}
                </h2>
                <div className="flex gap-1">
                  <button onClick={() => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() - 1))} className="p-1.5 sm:p-2 text-var(--text-muted) hover:text-var(--text-primary) hover:bg-var(--primary-glow) rounded-lg transition-colors"><ChevronLeft size={18} /></button>
                  <button onClick={() => setCurrentDate(new Date())} className="px-2 sm:px-3 py-1 text-[10px] sm:text-xs font-bold bg-var(--card-bg) text-var(--text-secondary) rounded-lg hover:bg-var(--primary-glow) transition-colors">Today</button>
                  <button onClick={() => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() + 1))} className="p-1.5 sm:p-2 text-var(--text-muted) hover:text-var(--text-primary) hover:bg-var(--primary-glow) rounded-lg transition-colors"><ChevronRight size={18} /></button>
                </div>
              </div>
              <div className="flex bg-var(--card-bg) p-1 rounded-xl border border-var(--card-border) w-full sm:w-auto">
                <button onClick={() => setViewMode('month')} className={`flex-1 sm:flex-none px-4 py-2 text-[10px] sm:text-xs font-bold rounded-lg transition-all ${viewMode === 'month' ? 'bg-var(--primary-glow) text-var(--primary) border border-var(--primary) shadow-sm' : 'text-var(--text-muted) border border-transparent'}`}>Calendar</button>
                <button onClick={() => setViewMode('list')} className={`flex-1 sm:flex-none px-4 py-2 text-[10px] sm:text-xs font-bold rounded-lg transition-all ${viewMode === 'list' ? 'bg-var(--primary-glow) text-var(--primary) border border-var(--primary) shadow-sm' : 'text-var(--text-muted) border border-transparent'}`}>List</button>
              </div>
            </div>

            <div className="calendar-grid grid grid-cols-7 gap-1 sm:gap-3">
              {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(d => (
                <div key={d} className="calendar-header-day text-center py-2 text-[8px] sm:text-[10px] font-bold text-var(--text-muted) uppercase tracking-widest">{d}</div>
              ))}
              
              {/* Empty cells */}
              {Array.from({ length: getFirstDayOfMonth(currentDate.getFullYear(), currentDate.getMonth()) }).map((_, i) => (
                <div key={`empty-${i}`} className="empty-day h-24 sm:h-36 bg-var(--card-bg) rounded-xl sm:rounded-2xl" />
              ))}
              ...
              {Array.from({ length: getDaysInMonth(currentDate.getFullYear(), currentDate.getMonth()) }).map((_, i) => {
                ...
                return (
                  <div key={day} className={`calendar-day h-24 sm:h-36 p-1 sm:p-3 rounded-xl sm:rounded-2xl transition-all group relative overflow-hidden flex flex-col ${isToday ? 'bg-var(--primary-glow) shadow-[inset_0_0_20px_rgba(var(--primary-rgb),0.15)]' : 'bg-var(--card-bg) hover:bg-var(--primary-glow)'}`}>
                    <div className="flex justify-between items-start">
                      <span className={`text-[10px] sm:text-sm font-bold ml-1 ${isToday ? 'text-var(--primary)' : 'text-var(--text-muted)'}`}>{day}</span>
                      {isBlocked && <Lock size={10} className="text-var(--text-muted) mt-0.5 sm:mt-1 mr-0.5 sm:mr-1" />}
                    </div>
          ...
          <div className="space-y-6">
             <div className="flex items-center justify-between">
                <h2 className="text-xl sm:text-2xl font-medium text-var(--text-primary) serif">Upcoming Sessions</h2>
                <button onClick={() => setViewMode('month')} className="text-[10px] font-bold text-var(--primary) uppercase tracking-widest hover:opacity-80">Switch to Calendar</button>
             </div>
             <div className="space-y-3 sm:space-y-4">
               {events.filter(e => new Date(e.date) >= new Date().setHours(0,0,0,0)).sort((a,b) => new Date(a.date) - new Date(b.date)).length > 0 ? 
                events.filter(e => new Date(e.date) >= new Date().setHours(0,0,0,0)).sort((a,b) => new Date(a.date) - new Date(b.date)).map((ev, idx) => (
                  <div key={idx} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 sm:p-5 rounded-2xl bg-var(--card-bg) border border-var(--card-border) hover:opacity-90 transition-all gap-4">
                    <div className="flex items-center gap-4 sm:gap-5">
                      <div className="w-12 h-12 sm:w-14 sm:h-14 bg-var(--primary-glow) rounded-xl flex flex-col items-center justify-center border border-var(--primary) text-var(--primary) shadow-inner shrink-0">
                      ...
                      <div className="min-w-0">
                        <p className="font-bold text-var(--text-primary) text-sm sm:text-base truncate">{ev.title}</p>
                        <p className="text-[11px] sm:text-xs text-var(--text-muted) font-medium mt-1">{ev.time} • {ev.topic || 'General'}</p>
                  ...
                  <div className="py-12 text-center border border-dashed border-var(--card-border) rounded-2xl">
                    <p className="text-var(--text-muted) font-medium text-sm">No upcoming sessions.</p>
                  </div>
                )}
             </div>
          </div>
      ...
      <div className="mt-6 flex flex-wrap gap-4 sm:gap-6 px-4">
        <div className="flex items-center gap-2 text-[10px] sm:text-[11px] font-bold text-var(--text-muted) uppercase tracking-wider">
      ...
      {showRequestModal && (
        <div className="modal-overlay">
          <div className="modal-content">
            <div className="flex items-center justify-between p-5 sm:p-6 border-b border-var(--card-border) bg-var(--primary-glow)">
              <div>
                ...
                <p className="text-[10px] sm:text-xs text-var(--text-muted) mt-1">Propose a time to your coach.</p>
              </div>
              <button 
                onClick={() => setShowRequestModal(false)}
                className="w-8 h-8 rounded-full bg-var(--card-bg) flex items-center justify-center text-var(--text-muted) hover:opacity-80 transition-colors"
              >
                <X size={16} />
              </button>
            </div>
              ...
              <div className="p-5 sm:p-6 space-y-4 sm:space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <label className="text-[9px] sm:text-[10px] font-bold text-var(--text-muted) uppercase tracking-widest pl-1">Date</label>
                    <input 
                      type="date" 
                  ...
                  <div className="space-y-2">
                    <label className="text-[9px] sm:text-[10px] font-bold text-var(--text-muted) uppercase tracking-widest pl-1">Time</label>
                    <select 
                ...
                <div className="space-y-2">
                  <label className="text-[9px] sm:text-[10px] font-bold text-var(--text-muted) uppercase tracking-widest pl-1">Focus Topic</label>
                  <input 
                ...
                <div className="space-y-2">
                  <label className="text-[9px] sm:text-[10px] font-bold text-var(--text-muted) uppercase tracking-widest pl-1">Note (Optional)</label>
                  <textarea 
                ...
                <div className="flex items-start gap-3 p-3 sm:p-4 rounded-xl bg-var(--primary-glow) border border-var(--primary) mt-2">
                  <AlertCircle size={14} className="text-var(--primary)" />
                  <p className="text-[11px] sm:text-[11.5px] text-var(--text-secondary) leading-relaxed font-light">
                    Your session is only confirmed once your coach accepts.
                  </p>
                </div>
              </div>

              <div className="p-5 sm:p-6 border-t border-var(--card-border) bg-var(--primary-glow) flex flex-col sm:flex-row justify-end gap-3">
